...
Api::Api(const std::string &url) : cli(url) { auth = false; }
...
std::string Api::getUserId(const std::string &name) {
  ...
  std::string path =
      httplib::append_query_params("/api/collections/profiles/records", qs);
...
void Api::createMail(std::string to, std::string subject, std::string content) {
  ...
  httplib::Result res =
      cli.Post("/api/collections/mails/records", getAuthHeaders(), body.c_str(),
...
void Api::setAuth(nlohmann::json auth) { this->auth = auth; }
...
void Api::login(const std::string &email, const std::string &password) {
  ...
  httplib::Result res = cli.Post("/api/users/auth-via-email", body.c_str(),
                                 body.size(), "application/json");
